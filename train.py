...
from wpdc_loss68 import WPDCLoss68
from creategraph import creategraphT as CRT
...
def parse_args():
    ...
    parser.add_argument('--arch', default='DFIN', type=str,
                        choices=arch_choices)                parser.add_argument('--frozen', default='false', type=str2bool)
    parser.add_argument('--milestones', default='30,50', type=str)
...
def train(train_loader, model, criterion, criterion1, criterion2, optimizer, epoch):
    ...
    for i, (input, img_r, target) in enumerate(train_loader):
        ...
        if args.loss.lower() == 'vdc':
            ...
            loss2 = criterion2(output3,target.view(-1,64,62))
        ...
        if i % args.print_freq == 0:
            logging.info(f'Epoch: [{epoch}][{i}/{len(train_loader)}]\t'
                         ...
                         f'Loss {losses1.val:.4f} ({losses1.avg:.4f}))\t'
                         f'Loss {losses2.val:.4f} ({losses2.avg:.4f}))\t')


def validate(val_loader, model, criterion, criterion1, criterion2, epoch):
    model.eval()
    ...
    with torch.no_grad():
        ...
        losses1 = []
        losses2 = []
        for i, (input, img_r, target) in enumerate(val_loader):
            # compute output
            target.requires_grad = False
            target = target.cuda(non_blocking=True)
            output, output1, output2 = model(input)
            output3=output2.view(-1,-1,62)
            loss = criterion(output, target)
            loss1 = criterion1(output1, target)
            loss2 = criterion2(output3, target)
...
def main():
    ...
    model = getattr(DFIN, args.arch)(num_classes=args.num_classes)  # ×*×*×*×*×*×*×*×*×*×*×*×*×*×*×*×*×*×
    # model = getattr(shufflenet, args.arch)(n_groups=args.n_groups)
    ...
    if args.loss.lower() == 'wpdc':
        ...
        criterion1 = WPDCLoss68(opt_style=args.opt_style).cuda()
        criterion2 = CRT(opt_style=args.opt_style).cuda()
        # criterion3 = MYLoss(opt_style=args.opt_style).cuda()
    ...
    for epoch in range(args.start_epoch, args.epochs + 1):
        ...
        validate(val_loader, model, criterion, criterion1, criterion2, epoch)
