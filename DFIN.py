...
class DFIN(nn.Module):
    ...
    dilated : bool, default False
        Applying dilation strategy to pretrained DFIN yielding a stride-8 model.
    ...
    def __init__(self, block, layers, groups=1, bottleneck_width=32,
                 num_classes=62, dilated=False, dilation=1,
        ...
        if dilated or dilation == 4:
            self.layer3 = self._make_layer2(block, 256, layers[2], stride=1,
        ...
        self.fc1_2 = nn.Linear(256 * block.expansion, 62 * block.expansion)
        self.fc2_2 = nn.Linear(256 * block.expansion, 62 * block.expansion)
        self.fc1_down = nn.Linear(248, 62)
        self.fc2_down = nn.Linear(248, 62)
        self.fc3 = nn.Linear(124 * block.expansion, num_classes)             #*****************************************************
    ...
    def _make_layer2(self, block, planes, blocks, stride=1, dilation=1, norm_layer=None,
                    is_first=True):
        downsample = None
        if stride != 1 or self.inplanes != planes * block.expansion:
            down_layers = []
            if self.avg_down:
                if dilation == 1:
                    ...
                    down_layers.append(nn.AvgPool2d(kernel_size=1, stride=1,
                                                    ceil_mode=True, count_include_pad=False))
                down_layers.append(DCN(self.inplanes, planes * block.expansion,
    ...
    def forward(self, x):
      ...
      #  x = self.fc(x)
        ...
        x1 = self.fc1(x)      #身份和表情   ***********************************************
        x2 = self.fc2(x)      #姿态
        x1 = self.fc1_2(x1)   #第二层身份和表情
        x2 = self.fc2_2(x2)   #第二层姿态
        #print(x1.size())
        #print(x2.size())
        x3 = torch.cat([x1,x2],1) #进行特征融合，把身份和表情特征进行融合，两个特征互相辅助
        #print(x3.size())
        x3 = self.fc3(x3)
        x1=self.fc1_down(x1)
        x2 = self.fc2_down(x2)
...
def DFIN(pretrained=False, **kwargs):

    model = DFIN(SCBottleneck, [3, 4, 6, 3],
