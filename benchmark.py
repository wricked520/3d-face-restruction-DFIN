...
# import os
# os.environ['CUDA_LAUNCH_DEVICE']="2
def extract_param(checkpoint_fp, root='', filelists=None, arch='', num_classes=62, device_ids=[0],
                  batch_size=64, num_workers=4):
    map_location = {f'cuda:{i}': 'cuda:0' for i in range(8)}
    #map_location = {f'cuda:{3}': 'cuda:3' }
    checkpoint = torch.load(checkpoint_fp, map_location=map_location)['state_dict']
    torch.cuda.set_device(device_ids[0])
    model = getattr(DFIN, arch)(num_classes=num_classes)
    ...
    with torch.no_grad():
        for _, inputs in enumerate(data_loader):
            ...
            inputs = inputs.cuda()
            output, output1, output2 = model(inputs)
            #print(inputs.size())
            for i in range(output.shape[0]):
                # param_prediction=[]
                # param_prediction1 = output[i].cpu().numpy().flatten()
                # param_prediction2 = output1[i].cpu().numpy().flatten()
                # param_prediction3 = output2[i].cpu().numpy().flatten()
                # param_prediction[:12] = param_prediction1[:12]
                # param_prediction[12:52] = param_prediction2[12:52]
                # param_prediction[52:] = param_prediction3[52:]
                # outputs.append(param_prediction)
                param_prediction = output2[i].cpu().numpy().flatten()
...
def benchmark_pipeline(arch, checkpoint_fp):
    ...
    def aflw():
        params = extract_param(
            ...
            root='/Data/test.data/AFLW_GT_crop',
            filelists='/Data/test.data/AFLW_GT_crop.list',
            arch=arch,
    ...
    def aflw2000():
        params = extract_param(
            checkpoint_fp=checkpoint_fp,
            root='/Data/test.data/AFLW2000-3D_crop',
            filelists='/Data/test.data/AFLW2000-3D_crop.list',
            arch=arch,
            device_ids=device_ids,
            batch_size=128)
